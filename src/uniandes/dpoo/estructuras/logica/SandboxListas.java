...
public class SandboxListas
{
    ...
    {
        Collections.sort(listaEnteros, new Comparator<Integer>() {
            @Override
            public int compare(Integer entero_1, Integer entero_2) {
                return entero_2.compareTo(entero_1);
            }
        });
    ...
    public int contarApariciones( String cadena )
    {
        ...
        for (String item : listaCadenas) {
            if (item.toLowerCase().equals(cadenaNueva)) {
                conteo++; 
            }
        }
    ...
    public int contarEnterosRepetidos( )
    {
        ...
        for (int valor : listaEnteros) {
            int i = valores.indexOf(valor);
            if (i == -1) {
                valores.add(valor);
                repetidos.add(1);
            } else {
                int nuevaFrecuencia = repetidos.get(i) + 1;
                repetidos.set(i, nuevaFrecuencia);
            }
        }

        int cantidadRepetidos = 0;
        for (int frecuencia : repetidos) {
            if (frecuencia > 1) {
                cantidadRepetidos++;
            }
        }
    ...
    public void generarEnteros( int cantidad, int minimo, int maximo )
    {
        ...
        if (cantidad <= 0 || minimo > maximo) {
            return; // No hacer nada si la cantidad es negativa o el rango es inválido
        }
